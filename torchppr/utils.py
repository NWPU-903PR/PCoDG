...
# 不知道为什么，MemoryUtilizationMaximizer代码有bug
# 比如indices长度为2时，换元素执行没什么问题
# 但增加元素到3时，会生成[2，5]的结果，元素增加到5个时生成[3,5]tensor
...
def prepare_x0(
    ...
    # 原版程序，相当于对每个节点单独游走，无法执行同时以多个节点为种子节点的游走
    # if indices is not None:
    #     k = len(indices)
    #     x0 = torch.zeros(n, k)
    #     x0[indices, torch.arange(k, device=x0.device)] = 1.0 #对角线元素置1
    #     return x0
    # 23-5-19更改，能够同时以多个节点为seeds游走
    if indices is not None:
        x0 = torch.zeros(n, 1)
        x0[indices, torch.arange(1, device=x0.device)] = 1.0 #对角线元素置1
        return x0
